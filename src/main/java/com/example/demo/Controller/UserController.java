...
public class UserController {
    ...
    @Operation(summary = "管理員本地端註冊", description = "使用 email、密碼、姓名與電話建立 ADMIN 帳號，並寄送信箱驗證碼。")
    @PostMapping("/api/admin/local-register")
    public String adminRegister(@Valid @RequestBody LocalRegisterRequest user) {
        return userService.registerLocal(user, "ADMIN");
    }
    ...
    @Operation(summary = "管理員 Google 註冊", description = "使用 Google credential 建立 ADMIN 帳號，並寄送信箱驗證碼。")
    @PostMapping("/api/admin/google-register")
    public String adminRegisterWithGoogle(@Valid @RequestBody GoogleCredentialRequest body) {
        return userService.registerGoogle(body, "ADMIN");
    }
    ...
    @Operation(summary = "管理員 Google 登入", description = "使用 Google credential 登入 ADMIN 帳號，成功後回傳 JWT token，並設定自動登出時間。")
    @PostMapping("/api/admin/google-login")
    public Map<String, Object> adminloginWithGoogle(@Valid @RequestBody GoogleCredentialRequest body) {
        return userService.loginGoogle(body, "ADMIN");
    }
    ...
    @Operation(summary = "驗證重設密碼信箱", description = "使用 email 與 6 位數驗證碼確認重設密碼流程中的信箱驗證碼是否有效。")
    ...
    @Operation(summary = "重設密碼", description = "使用 email 與新密碼重設本地端帳號密碼。")
