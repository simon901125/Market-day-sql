...
| status        | INDEX | -  | VARCHAR(30)     | 是   | 帳號狀態（預設 ACTIVE；ACTIVE/NOT_ACTIVE/IS_DELETED） |
...
- `status`：`ACTIVE`、`NOT_ACTIVE`、`IS_DELETED`。帳號建立後預設 `ACTIVE`，不做人為啟用審核。
...
| method            | -    | -                     | NVARCHAR(30)       | 是   | 付款方式     |
| provider          | -    | -                     | NVARCHAR(30) NULL  | 否   | 金流服務商   |
| provider_trade_no | -    | -                     | NVARCHAR(100) NULL | 否   | 金流交易編號 |
| virtual_account   | -    | -                     | NVARCHAR(100) NULL | 否   | ATM 虛擬帳號 |
| cvs_code          | -    | -                     | NVARCHAR(100) NULL | 否   | 超商付款代碼 |
