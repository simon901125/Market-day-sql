...
| 欄位名稱          | 鍵別 | FK            | 型別               | 必填 | 說明           |
| ----------------- | ---- | ------------- | ------------------ | ---- | -------------- |
| id                | PK   | -             | BIGINT IDENTITY    | 是   | 品牌 ID        |
| user_id           | FK   | users.id      | BIGINT             | 是   | 攤主使用者 ID  |
| category_id       | FK   | categories.id | BIGINT             | 是   | 分類 ID        |
| name              | -    | -             | NVARCHAR(150)      | 是   | 品牌名稱       |
| short_description | -    | -             | NVARCHAR(255)      | 是   | 品牌簡介       |
| description       | -    | -             | NVARCHAR(MAX)      | 是   | 品牌介紹       |
| instagram_url     | -    | -             | NVARCHAR(500) NULL | 否   | Instagram      |
| facebook_url      | -    | -             | NVARCHAR(500) NULL | 否   | Facebook       |
| website_url       | -    | -             | NVARCHAR(500) NULL | 否   | 官方網站       |
| contact_email     | -    | -             | NVARCHAR(255) NULL | 否   | 品牌聯絡 Email |
| contact_phone     | -    | -             | NVARCHAR(30) NULL  | 否   | 品牌聯絡電話   |
...
### 4.3 `vendor_images` 品牌圖片

用來存放品牌大頭照與品牌照片。照片不直接放在 `vendors`，是因為大頭照通常只有一張，但品牌照片可能多張；拆成獨立表比較好管理排序、替換與前台顯示。

| 欄位名稱   | 鍵別 | FK         | 型別            | 必填 | 說明     |
| ---------- | ---- | ---------- | --------------- | ---- | -------- |
| id         | PK   | -          | BIGINT IDENTITY | 是   | 照片 ID  |
| vendor_id  | FK   | vendors.id | BIGINT          | 是   | 品牌 ID  |
...
- SQL Server filtered unique index：`UNIQUE(vendor_id) WHERE image_type = 'AVATAR'`，限制每個品牌只能有一張大頭貼 / LOGO。
- SQL Server filtered unique index：`UNIQUE(vendor_id) WHERE image_type = 'COVER'`，限制每個品牌只能有一張封面。
- `image_type = GALLERY` 可保留多張，用於前台品牌詳細與攤主後台我的品牌。

### 4.4 `vendor_products` 品牌商品
